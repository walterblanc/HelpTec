...
Public Class Form70
    ...
    Private Sub Form70_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub inicializo_formulario()
        ...
        cargar_cuentas_bancarias(MetroComboBox2)
    ...
    End Sub
    Private Sub cargar_cuentas_bancarias(ByRef Lista As ComboBox)
        ...
        Try
            ...
            Dim cmdCombo As New OleDbCommand("Select * from Cuentas_Bancarias WHERE (Estado=0) Order by Detalle", ConSql)
    ...
    Private Sub cargo_Cliente()
        ...
        Try
            ...
            ConSql.Open()
            Dim Sql As String = ""
            Sql = "Select * from Proveedores WHERE (Numero= " & reser_Cliente & ") And (Estado=0) "
    ...
    Private Sub cargar_datagrid()
        ...
        Dim xSaldo_Con_Retencion As Double = 0
        Dim xSaldo_Sin_Retencion As Double = 0
        Dim xSaldoInicial_Con_Retencion As Double = 0
        Dim xSaldoInicial_Sin_Retencion As Double = 0

        Call saldo_cuenta_corriente(reser_Cliente, FormatDateTime(MetroDateTime1.Text, DateFormat.ShortDate), _
                                    FormatDateTime(MetroDateTime2.Text, DateFormat.ShortDate), 2, 1, _
                                     xSaldoDeudor, xSaldoAcredor)



        xSaldoInicial_Con_Retencion = xSaldoDeudor - xSaldoAcredor
        xSaldo_Con_Retencion = xSaldoInicial_Con_Retencion

        Call saldo_cuenta_corriente(reser_Cliente, FormatDateTime(MetroDateTime1.Text, DateFormat.ShortDate), _
                                    FormatDateTime(MetroDateTime2.Text, DateFormat.ShortDate), 2, 0, _
                                     xSaldoDeudor, xSaldoAcredor)

        xSaldoInicial_Sin_Retencion = xSaldoDeudor - xSaldoAcredor
        xSaldo_Sin_Retencion = xSaldoInicial_Sin_Retencion


        MetroTextBox3.Text = FormatNumber(xSaldoInicial_Sin_Retencion, 2)
        MetroTextBox24.Text = FormatNumber(xSaldoInicial_Con_Retencion, 2)


        Sql = "Select * from Cuenta_corriente_proveedores Where (Estado = 0) AND (Cliente=" & reser_Cliente & ") "
        ...
        With metroGrid1
            ...
            .Columns.Add("J", "Reten.")
            .Columns.Add("K", "Tot.Comp.")
            .Columns.Add("M", "Saldo")
            .Columns.Add("N", "Sal.C/Ret.")
            .Columns.Add("L", "Desc.Tomado")
        ...
        If drRecordSet.HasRows Then 'Tiene filas
            Do While drRecordSet.Read
                ...
                If Val(drRecordSet("CodigoMovimiento").ToString) < 4 Then
                    ...
                    '                    xSaldoComprobante = Math.Round((Convert.ToDouble(drRecordSet("Importe").ToString) - Convert.ToDouble(drRecordSet("Retenciones").ToString)) - xCancelado, 2)
                ...
                If Val(drRecordSet("DebCre").ToString) = 1 Then
                    xSaldo_Sin_Retencion = xSaldo_Sin_Retencion + Convert.ToDouble(drRecordSet("Importe").ToString)
                    xSaldo_Con_Retencion = xSaldo_Con_Retencion + (Convert.ToDouble(drRecordSet("Importe").ToString) - Convert.ToDouble(drRecordSet("Retenciones").ToString))
                Else
                    xSaldo_Sin_Retencion = xSaldo_Sin_Retencion - Convert.ToDouble(drRecordSet("Importe").ToString)
                    xSaldo_Con_Retencion = xSaldo_Con_Retencion - Convert.ToDouble(drRecordSet("Importe").ToString)
                End If
                ...
                If mostrar = True Then
                    With metroGrid1
                        ...
                        If Val(drRecordSet("CodigoMovimiento").ToString) < 4 Then
                            .Item(7, h).Value = FormatNumber(xSaldoComprobante, 2)
                        End If
                        .Item(8, h).Value = FormatNumber(Convert.ToDouble(drRecordSet("Retenciones").ToString), 2)
                        .Item(9, h).Value = FormatNumber(Convert.ToDouble(drRecordSet("Importe").ToString), 2)

                        .Item(10, h).Value = FormatNumber(xSaldo_Sin_Retencion, 2)
                        .Item(11, h).Value = FormatNumber(xSaldo_Con_Retencion, 2)
        ...
        MetroTextBox22.Text = FormatNumber(xSaldo_Sin_Retencion, 2)
        MetroTextBox23.Text = FormatNumber(xSaldo_Con_Retencion, 2)
    ...
    Public Function saldo_comprobante(ByVal xCuenta As Double, ByVal xMovim As Byte, ByVal xLetra As String, ByVal xnumero As String) As Double
        ...
        Try
            ...
            Dim Sql As String = ""
            Sql = "Select * from Cancelaciones_corriente_proveedores Where (Estado = 0) "
    ...
    Private Sub saldo_cuenta_corriente(ByVal xCuenta As Double, ByVal xFecha1 As String, ByVal xFecha2 As String, ByVal xTipo As Byte, _
                              ByVal xCon_Retencion As Byte, ByRef xSaldoDeudor As Double, ByRef xSaldoAcredor As Double)
        ...
        Try
            ...
            If xCon_Retencion = 0 Then
                If xTipo = 0 Then
                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=1) AND (Estado=0)"
                    xImp_Debitos = calculos_sql(InsSql)

                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=2) AND (Estado=0)"
                    xImp_Creditos = calculos_sql(InsSql)
                End If

                If xTipo = 1 Then
                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=1) AND (Estado=0) AND (Fecha >= '" & xFecha1 & "') AND (Fecha <= '" & xFecha2 & "') "
                    xImp_Debitos = calculos_sql(InsSql)

                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=2) AND (Estado=0) AND (Fecha >= '" & xFecha1 & "') AND (Fecha <= '" & xFecha2 & "') "
                    xImp_Creditos = calculos_sql(InsSql)
                End If

                If xTipo = 2 Then
                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=1) AND (Estado=0) AND (Fecha < '" & xFecha1 & "') "
                    xImp_Debitos = calculos_sql(InsSql)

                    InsSql = "Select Sum (importe) as Expr from Cuenta_corriente_proveedores Where (Cliente =  " & xCuenta & ") AND (Debcre=2) AND (Estado=0) AND (Fecha < '" & xFecha1 & "') "
                    xImp_Creditos = calculos_sql(InsSql)
                End If
            End If

            If xCon_Retencion = 1 Then
                If xTipo = 0 Then
    ...
    Private Sub MetroButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton1.Click
        ...
        definir_grilla_comprobantes()
        suma_grilla()
        refrescar_indicadores()
    End Sub
    Private Sub refrescar_indicadores()
        MetroTextBox4.Text = "0.00"
        MetroTextBox5.Text = "0.00"
        MetroTextBox6.Text = "0.00"
        MetroTextBox7.Text = "0.00"
    ...
    Private Sub metroTextBox5_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MetroTextBox5.KeyPress
        ...
        End If
    End Sub
    Private Sub imputar_por_lotes()


        Dim selectedRowCount As Integer = metroGrid1.Rows.GetRowCount(DataGridViewElementStates.Selected)
        ...
        If selectedRowCount > 0 Then
            ...
            For i = 0 To selectedRowCount - 1
                If duplicados_en_grilla_b(i) = False Then
                    If metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim <> "RE" Then
                        If metroGrid1.SelectedRows(i).Cells(7).Value.ToString.Trim <> "" Then
                            If Convert.ToDouble(metroGrid1.SelectedRows(i).Cells(7).Value.ToString.Trim) <> 0 Then
                                With MetroGrid2
                                    .Rows.Add()
                                    .Item(0, fila).Value = metroGrid1.SelectedRows(i).Cells(0).Value.ToString.Trim
                                    .Item(1, fila).Value = metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim
                                    .Item(2, fila).Value = metroGrid1.SelectedRows(i).Cells(2).Value.ToString.Trim
                                    .Item(3, fila).Value = metroGrid1.SelectedRows(i).Cells(3).Value.ToString.Trim
                                    .Item(4, fila).Value = metroGrid1.SelectedRows(i).Cells(4).Value.ToString.Trim

                                    If metroGrid1.SelectedRows(i).Cells(5).Value.ToString.Trim = "" Then
                                        .Item(5, fila).Value = metroGrid1.SelectedRows(i).Cells(6).Value.ToString.Trim

                                        Aux = CDbl(metroGrid1.SelectedRows(i).Cells(6).Value.ToString.Trim)

                                    Else
                                        .Item(5, fila).Value = metroGrid1.SelectedRows(i).Cells(5).Value.ToString.Trim

                                        Aux = CDbl(metroGrid1.SelectedRows(i).Cells(5).Value.ToString.Trim)

                                    End If


                                    .Item(6, fila).Value = FormatNumber(CDbl(metroGrid1.SelectedRows(i).Cells(7).Value.ToString.Trim), 2)
                                    .Item(7, fila).Value = FormatNumber(CDbl(metroGrid1.SelectedRows(i).Cells(7).Value.ToString.Trim), 2)
                                    fila = fila + 1
                                End With
                            End If
                        End If
    ...
    End Sub
    Private Function duplicados_en_grilla_b(ByVal x As Integer) As Boolean
        Dim r As Boolean = False
        Try
            For Fila_pag As Integer = 0 To MetroGrid2.Rows.Count - 1

                If MetroGrid2.Rows(Fila_pag).Cells(0).Value = metroGrid1.SelectedRows(x).Cells(0).Value.ToString.Trim And _
                    MetroGrid2.Rows(Fila_pag).Cells(1).Value = metroGrid1.SelectedRows(x).Cells(1).Value.ToString.Trim And _
                    MetroGrid2.Rows(Fila_pag).Cells(2).Value = metroGrid1.SelectedRows(x).Cells(2).Value.ToString.Trim And _
                    MetroGrid2.Rows(Fila_pag).Cells(3).Value = metroGrid1.SelectedRows(x).Cells(3).Value.ToString.Trim Then
                    r = True
                End If
            Next
        Catch ex As Exception
    ...
    Private Sub metroGrid1_CellMouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles metroGrid1.CellMouseDoubleClick
        ...
        If metroGrid1.Rows(indice).Cells(7).Value.trim = "" Then
            ...
            Exit Sub
        End If
        If Convert.ToDouble(metroGrid1.Rows(indice).Cells(7).Value.trim) = 0 Then
            MetroFramework.MetroMessageBox.Show(Me, "El Comprobante ya fue Imputado", rsv_Modulo, MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        ...
        With MetroGrid2
            ...
            .Item(6, fila).Value = FormatNumber(CDbl(metroGrid1.Rows(indice).Cells(7).Value), 2)
            .Item(7, fila).Value = FormatNumber(CDbl(metroGrid1.Rows(indice).Cells(7).Value), 2)
            fila = fila + 1
        End With
    ...
    Private Sub MetroTextBox4_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox4.Validated
        ...
        End If
        calcular_pago_efectivo()
        calcular_importe_recibo()

    End Sub

    Private Sub MetroTextBox6_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox6.Validated
        ...
        calcular_importe_recibo()
    End Sub
    Private Sub MetroTextBox8_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox8.Validated
        ...
        End If
    End Sub
    Private Sub MetroTextBox5_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox5.Validated
        ...
        End If
    End Sub
    Private Sub MetroTextBox10_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox10.Validated
        ...
        End If
    End Sub
    Private Sub MetroTextBox7_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox7.Validated
    ...
    Private Sub MetroButton7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton7.Click
        ...
        Dim x As String = CType(MetroComboBox2.SelectedItem, ValueDescriptionPair).Value
        Try
            With MetroGrid3
                .Rows.Add()
    ...
    Private Sub suma_grilla_valores()
        Try
            ...
            MetroTextBox5.Text = FormatNumber(x, 2)
        Catch ex As Exception
            MessageBox.Show(ex.Message.ToString, rsv_NombreModulo, MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End Try
    End Sub
    ...
    Private Sub MetroButton9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton9.Click
        Try
            ...
            End If
            If MetroTextBox15.Text.Trim <> "" Then
                MetroTextBox15.Text = FormatNumber(CDbl(MetroTextBox15.Text.Trim), 2)
            End If
            ...
            If Math.Round(Convert.ToDouble(MetroTextBox14.Text.Trim), 2) <> Math.Round(Convert.ToDouble(MetroTextBox16.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox20.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox21.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox6.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox5.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox15.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox4.Text.Trim) + _
                                                             Convert.ToDouble(MetroTextBox7.Text.Trim), 2) Then
                MetroFramework.MetroMessageBox.Show(Me, "La Suma de los de pagos no es igual al total del recibo", rsv_Modulo, MessageBoxButtons.OK, MessageBoxIcon.Information)
            ...
            Aux1 = Math.Round(CDbl(MetroTextBox14.Text.Trim), 2)
            If Aux1 = 0 Then
            ...
            If Convert.ToDouble(MetroTextBox15.Text.Trim) <> 0 Then
                guardar_valores_terceros()
            End If

            Dim largo As Boolean = False

            Call impresion_recibos_proveedores(Formatea_Numero_Comprobante(0, CDbl(MetroTextBox13.Text.Trim)), reser_Cliente, largo)

            If largo = True Then
                Call impresion_recibos_proveedores_largo(Formatea_Numero_Comprobante(0, CDbl(MetroTextBox13.Text.Trim)), reser_Cliente)
            End If
    ...
    Private Sub guardar_valores()
        Try
            ...
            For Fila As Integer = 0 To MetroGrid3.Rows.Count - 1
                ...
                x12 = MetroGrid3.Rows(Fila).Cells("Id").Value

                InsSql = ""
                InsSql = "Insert into Valores_Emitidos ("
                InsSql = InsSql & "Fecha,LetraComprobante,NumeroComprobante,Cuenta,Cliente,TipoComprobante,"
                InsSql = InsSql & "Banco,Emision,Pago,Numero,Importe,Detalle,"
                InsSql = InsSql & "Ua,Ha,Fa,Estado) Values ("
                InsSql = InsSql & "'" & x1 & "','" & x2 & "','" & x3 & "','" & x12 & "'," & x4 & "," & x5 & ","
    ...
    End Sub
    Private Sub guardar_valores_terceros()
        Try
            ...
            For i = 0 To rsv_arr.Length - 1
                ...
                If s <> 0 Then
                    lectura_valores_terceros_guardar(s)
                    marcar_valores_entregados(s)
                End If
            Next
    ...
    Private Sub grabar_cancelaciones()
        Try
            ...
            For Fila As Integer = 0 To MetroGrid2.Rows.Count - 1
                ...
                Dim InsSql As String = ""
                InsSql = ""
                InsSql = "Insert into Cancelaciones_corriente_proveedores ("
    ...
    Private Sub registro_cuenta_corriente()
        Try
            ...
            x10 = Convert.ToDouble(MetroTextBox16.Text.Trim)
            x11 = Convert.ToDouble(MetroTextBox20.Text.Trim)
            x12 = Convert.ToDouble(MetroTextBox21.Text.Trim)
            ...
            InsSql = InsSql & "Cliente,Fecha,FechaImputado,DebCre,CodigoMovimiento,LetraComprobante,NumeroComprobante,Importe,Detalle,Descuentos,Devoluciones,RetencionesPago,Efect,ChePro,CheTer,Transf,Tarj,"
            InsSql = InsSql & "UsuarioAlta,FechaAlta,HoraAlta,Estado) Values ("
            InsSql = InsSql & "" & x1 & ",'" & x2 & "','" & x3 & "'," & x4 & "," & x5 & ",'" & x6 & "','" & x7 & "'," & x8 & ",'" & x9 & "'," & x10 & "," & x11 & "," & x12 & "," & x13 & "," & x14 & "," & x15 & "," & x16 & "," & x17 & ","
            InsSql = InsSql & "" & j1 & ",'" & j2 & "','" & j3 & "',0)"
            Dim cmdActualizar As New OleDbCommand(InsSql, ConSql)
            cmdActualizar.ExecuteNonQuery()
            cmdActualizar.Dispose()
    ...
    Private Sub registro_recibo()
        Try
            ...
            x6 = Convert.ToDouble(MetroTextBox5.Text.Trim) + Convert.ToDouble(MetroTextBox15.Text.Trim)
            ...
            Dim InsSql As String = ""
            InsSql = ""
            InsSql = "Insert into Recibos_proveedores ("
            InsSql = InsSql & "Recibo,Letra,Fecha,Cliente,Efectivo,Cheques,Tarjeta,IdTarjeta,Transferencia,Descuentos,Devoluciones,RetencionesPago,"
            InsSql = InsSql & "Ua,Fa,Ha,Estado) Values ("
            InsSql = InsSql & "'" & x1 & "','" & x2 & "','" & x3 & "'," & x4 & "," & x5 & "," & x6 & "," & x7 & "," & x8 & "," & x9 & "," & x10 & "," & x11 & "," & x12 & ","
            InsSql = InsSql & "" & j1 & ",'" & j2 & "','" & j3 & "',0)"
            Dim cmdActualizar As New OleDbCommand(InsSql, ConSql)
            cmdActualizar.ExecuteNonQuery()
            cmdActualizar.Dispose()
    ...
    Private Sub MetroButton10_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton10.Click
        ...
        If (metroGrid1.CurrentRow IsNot Nothing) Then
            ...
            Call impresion_recibos_proveedores(metroGrid1.CurrentRow.Cells(3).Value.ToString.Trim, reser_Cliente, largo)

            If largo = True Then
                Call impresion_recibos_proveedores_largo(metroGrid1.CurrentRow.Cells(3).Value.ToString.Trim, reser_Cliente)
            End If
    ...
    Private Sub MetroButton11_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton11.Click
        Dim Formulario_open As New Form73(reser_Cliente, Convert.ToDateTime(FormatDateTime(MetroDateTime1.Text, DateFormat.ShortDate)), Convert.ToDateTime(FormatDateTime(MetroDateTime2.Text, DateFormat.ShortDate)))
    ...
    Private Sub metroGrid1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles metroGrid1.KeyDown
        ...
        If e.KeyCode = Keys.F10 Then
            ...
            Dim xFecha As String = metroGrid1.Rows(li_index).Cells(0).Value
            Dim xTipo As String = metroGrid1.Rows(li_index).Cells(1).Value
            Dim xLetra As String = metroGrid1.Rows(li_index).Cells(2).Value
            Dim xNumero As String = metroGrid1.Rows(li_index).Cells(3).Value
    ...
    Private Sub MetroButton12_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton12.Click
        Dim Formulario_open As New Form72
        Formulario_open.ShowDialog()


        If rsv_arr.Length >= 0 Then
            lectura_valores_seleccionados()
        End If
    ...
    Private Sub lectura_valores_seleccionados()
        Try
            ...
            For i = 0 To rsv_arr.Length - 1
                s = Convert.ToDouble(rsv_arr(i))
                If s <> 0 Then
                    Acum = Acum + lectura_valores_terceros(s)
                End If
            Next

            MetroTextBox15.Text = FormatNumber(Acum, 2)
    ...
    Private Function lectura_valores_terceros(ByVal xId As Double) As Double
        Dim XImporte As Double = 0
        Try
            If ConSql.State = ConnectionState.Closed Then
                ConSql.Open()
            ...
            Dim Sql As String = ""
            Sql = "Select * from Valores_recibidos_ventas WHERE Id = " & xId & " "

            Dim cmdRecordSet As New OleDbCommand(Sql, ConSql)
            Dim drRecordSet As OleDbDataReader = cmdRecordSet.ExecuteReader
            If drRecordSet.HasRows Then 'Tiene filas
                If drRecordSet.Read Then
                    XImporte = Convert.ToDouble(drRecordSet("Importe").ToString)
                End If
            End If
        ...
        Catch ex As Exception
            MessageBox.Show(ex.Message.ToString, rsv_NombreModulo, MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Finally
            lectura_valores_terceros = XImporte
        End Try
    End Function

    Private Sub lectura_valores_terceros_guardar(ByVal xId As Double)
        Try
            ...
            If drRecordSet.HasRows = True Then 'Tiene filas
                Do While drRecordSet.Read
                    x6 = drRecordSet("Banco").ToString
                    x9 = drRecordSet("Numero").ToString
                    x7 = FormatDateTime(drRecordSet("Emision").ToString, DateFormat.ShortDate)
                    x8 = FormatDateTime(drRecordSet("Pago").ToString, DateFormat.ShortDate)
                    x10 = Convert.ToDouble(drRecordSet("Importe").ToString)
                    x11 = drRecordSet("Detalle").ToString
                    x12 = Convert.ToDouble(drRecordSet("Cliente").ToString)


                    Sql = ""
                    Sql = "Insert into Valores_Emitidos ("
                    Sql = Sql & "Fecha,LetraComprobante,NumeroComprobante,Cliente,TipoComprobante,"
                    Sql = Sql & "Banco,Emision,Pago,Numero,Importe,Detalle,OrigenCliente,ChequeTercero,"
                    Sql = Sql & "Ua,Ha,Fa,Estado) Values ("
                    Sql = Sql & "'" & x1 & "','" & x2 & "','" & x3 & "'," & x4 & "," & x5 & ","
                    Sql = Sql & "" & x6 & ",'" & x7 & "','" & x8 & "'," & x9 & "," & x10 & ",'" & x11 & "'," & x12 & ",1,"
                    Sql = Sql & "" & j1 & ",'" & j3 & "','" & j2 & "',0)"
        ...
        End Try
    End Sub
    Private Sub marcar_valores_entregados(ByVal xId As Double)
        Try
            ...
            Sql = "Update Valores_recibidos_ventas SET PagoProveedor=1,"
            Sql = Sql & "FechaPagoProveedor='" & j2 & "',HoraPagoProveedor='" & j3 & "',UsuarioPagoProveedor=" & j1 & " WHERE ID= " & xId & " "
    ...
    Private Sub MetroComboBox2_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroComboBox2.SelectedIndexChanged
        Try
            Dim x As String = CType(MetroComboBox2.SelectedItem, ValueDescriptionPair).Value
            If ConSql.State = ConnectionState.Closed Then
                ConSql.Open()
            ...
            Dim Sql As String = ""
            Sql = "Select * from Cuentas_Bancarias WHERE (Numero= '" & x & "') And (Estado=0) "
            ...
            If drRecordSet.HasRows Then 'Tiene filas
                If drRecordSet.Read Then
                    MetroTextBox11.Text = drRecordSet("Banco").ToString
                End If
            End If
    ...
    Private Sub MetroLink2_Click(sender As System.Object, e As System.EventArgs) Handles MetroLink2.Click
        imputar_por_lotes()
    End Sub


    Private Sub CuentosToolStripMenuItem_Click(sender As System.Object, e As System.EventArgs) Handles CuentosToolStripMenuItem.Click
        If metroGrid1.Rows.Count <= 0 Then
            Exit Sub
        End If

        If (metroGrid1.CurrentRow IsNot Nothing) Then
            Dim Letra As String = metroGrid1.CurrentRow.Cells(2).Value
            Dim Tipo As String = metroGrid1.CurrentRow.Cells(1).Value
            Dim Numero As String = metroGrid1.CurrentRow.Cells(3).Value

            If Tipo <> "FA" And Tipo <> "NC" Then
                Exit Sub
            End If

            If Val(metroGrid1.CurrentRow.Cells(12).Value) = 1 Then
                MetroFramework.MetroMessageBox.Show(Me, "Descuento Aplicado", rsv_Modulo, MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If



            Dim Formulario_open As New Form71(reser_Cliente, Letra, Tipo, Numero)
            Formulario_open.ShowDialog()
            ...
            If rsv_Numero1 + rsv_Numero2 + rsv_Numero3 = 0 Then
                Exit Sub
            End If

            If Tipo = "FA" Then
                'Descuentos
                MetroTextBox16.Text = Convert.ToDouble(MetroTextBox16.Text) + (Convert.ToDouble(metroGrid1.CurrentRow.Cells(9).Value) - rsv_Numero1)
                MetroTextBox16.Text = FormatNumber(Convert.ToDouble(MetroTextBox16.Text), 2)

                'Devoluciones
                MetroTextBox20.Text = Convert.ToDouble(MetroTextBox20.Text) + rsv_Numero3
                MetroTextBox20.Text = FormatNumber(Convert.ToDouble(MetroTextBox20.Text), 2)

                'Retenciones
                MetroTextBox21.Text = Convert.ToDouble(MetroTextBox21.Text) + rsv_Numero2
                MetroTextBox21.Text = FormatNumber(Convert.ToDouble(MetroTextBox21.Text), 2)
            End If
            ...
            metroGrid1.CurrentRow.Cells(12).Value = "1"
    ...
    Private Sub MetroContextMenu1_Opening(sender As System.Object, e As System.ComponentModel.CancelEventArgs) Handles MetroContextMenu1.Opening
    ...
    End Sub
    Private Sub calcular_pago_efectivo()
        If MetroTextBox16.Text.Trim = "" Then
            MetroTextBox16.Text = "0.00"
        End If
        If MetroTextBox20.Text.Trim = "" Then
        ...
        'Aux = Convert.ToDouble(MetroTextBox18.Text) - Convert.ToDouble(MetroTextBox5.Text) - _
        '                     Convert.ToDouble(MetroTextBox15.Text) - Convert.ToDouble(MetroTextBox4.Text) - _
        '                     Convert.ToDouble(MetroTextBox7.Text) - Convert.ToDouble(MetroTextBox16.Text) - _
        '                     Convert.ToDouble(MetroTextBox20.Text) - Convert.ToDouble(MetroTextBox21.Text)

        'Modificacion por acumulacion de retenciones en el descuento
        Aux = Convert.ToDouble(MetroTextBox18.Text) - Convert.ToDouble(MetroTextBox5.Text) - _
                             Convert.ToDouble(MetroTextBox15.Text) - Convert.ToDouble(MetroTextBox4.Text) - _
                             Convert.ToDouble(MetroTextBox7.Text) - Convert.ToDouble(MetroTextBox16.Text) - _
                             Convert.ToDouble(MetroTextBox20.Text)



        MetroTextBox6.Text = FormatNumber(Aux, 2)


    End Sub
    Private Sub descuentos_por_lote()

        Dim xDesc As Double = 0
        Dim xDevol As Double = 0
        Dim xReten As Double = 0

        Dim xAcum1 As Double = 0
        Dim xAcum2 As Double = 0
        Dim xAcum3 As Double = 0
        ...
        If selectedRowCount > 0 Then
            ...
            Dim i As Integer
            For i = 0 To selectedRowCount - 1
                If metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim = "FA" Or metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim = "NC" Then
                    With MetroGrid2
                        xDesc = 0
                        xDevol = 0

                        Call retorna_descuentos(metroGrid1.SelectedRows(i).Cells(0).Value.ToString.Trim, metroGrid1.SelectedRows(i).Cells(2).Value.ToString.Trim, _
                                                metroGrid1.SelectedRows(i).Cells(3).Value.ToString.Trim, metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim, _
                                                xDesc, xDevol, xReten)


                        If metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim = "FA" Then

                            xAcum1 = xAcum1 + (Convert.ToDouble(metroGrid1.SelectedRows(i).Cells(9).Value.ToString.Trim) - xDesc)
                            xAcum2 = xAcum2 + xDevol
                            xAcum3 = xAcum3 + xReten

                        End If

                        If metroGrid1.SelectedRows(i).Cells(1).Value.ToString.Trim = "NC" Then
                        ...
                        metroGrid1.SelectedRows(i).Cells(12).Value = "1"
        ...
        MetroTextBox16.Text = FormatNumber(xAcum1, 2)
        MetroTextBox20.Text = FormatNumber(xAcum2, 2)
        MetroTextBox21.Text = FormatNumber(xAcum3, 2)


    End Sub
    Private Sub retorna_descuentos(ByVal xFecha As String, ByVal xLetra As String, ByVal xNumero As String, _
                                   ByVal xTipo As String, ByRef xDesc As Double, ByRef xDevol As Double, ByRef xReten As Double)

        Dim xTipo_Red As Byte = 1
        Dim xAcum1 As Double = 0
        Dim xAcum2 As Double = 0
        Dim xAcum3 As Double = 0
        ...
        Try
            Dim sql As String = ""
            sql = "Select * from Cuenta_corriente_proveedores WHERE (Estado = 0) AND (Cliente=" & reser_Cliente & ") AND (CodigoMovimiento=" & xTipo_Red & ") "
            sql = sql & "AND (Fecha = '" & xFecha & "') AND (LetraComprobante= '" & xLetra & "') AND (NumeroComprobante= '" & xNumero & "') "
            ...
            If drRecordSet.HasRows Then 'Tiene filas
                If drRecordSet.Read Then
                    xAcum1 = xAcum1 + Convert.ToDouble(drRecordSet("T1").ToString)
                    xAcum2 = xAcum2 + Convert.ToDouble(drRecordSet("T2").ToString)
                    xAcum3 = xAcum3 + Convert.ToDouble(drRecordSet("Retenciones").ToString)
                End If
            End If
    ...
    Private Sub MetroLink1_Click(sender As System.Object, e As System.EventArgs) Handles MetroLink1.Click
        desmarcar_descuentos()
        descuentos_por_lote()
        calcular_importe_recibo()
    End Sub

    Private Sub desmarcar_descuentos()
        Try
            For Fila As Integer = 0 To metroGrid1.Rows.Count - 1
                metroGrid1.Rows(Fila).Cells("L").Value = ""
            Next
        Catch ex As Exception
            MessageBox.Show(ex.Message.ToString, rsv_NombreModulo, MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End Try
    ...
    Private Sub MetroButton14_Click(sender As System.Object, e As System.EventArgs) Handles MetroButton14.Click
        ...
        calcular_importe_recibo()
    End Sub
    Private Sub calcular_importe_recibo()
        If MetroTextBox16.Text.Trim = "" Then
            MetroTextBox16.Text = "0.00"
        End If
        If MetroTextBox20.Text.Trim = "" Then
        ...
        '        Aux = Convert.ToDouble(MetroTextBox16.Text.Trim) + Convert.ToDouble(MetroTextBox20.Text.Trim) + Convert.ToDouble(MetroTextBox21.Text.Trim) + Convert.ToDouble(MetroTextBox5.Text.Trim) + Convert.ToDouble(MetroTextBox15.Text.Trim) + Convert.ToDouble(MetroTextBox4.Text.Trim) + Convert.ToDouble(MetroTextBox7.Text.Trim) + Convert.ToDouble(MetroTextBox6.Text.Trim)
        Aux = Convert.ToDouble(MetroTextBox16.Text.Trim) + Convert.ToDouble(MetroTextBox20.Text.Trim) + Convert.ToDouble(MetroTextBox5.Text.Trim) + Convert.ToDouble(MetroTextBox15.Text.Trim) + Convert.ToDouble(MetroTextBox4.Text.Trim) + Convert.ToDouble(MetroTextBox7.Text.Trim) + Convert.ToDouble(MetroTextBox6.Text.Trim)
        MetroTextBox14.Text = FormatNumber(Aux, 2)

    End Sub
    Private Sub MetroTextBox16_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox16.Validated
        calcular_pago_efectivo()
        calcular_importe_recibo()
    End Sub

    Private Sub MetroTextBox20_Click(sender As System.Object, e As System.EventArgs) Handles MetroTextBox20.Click
        calcular_pago_efectivo()
        calcular_importe_recibo()
    End Sub
    ...
    Private Sub MetroTextBox21_Validated(sender As Object, e As System.EventArgs) Handles MetroTextBox21.Validated
        calcular_pago_efectivo()
        calcular_importe_recibo()
    End Sub

    Private Sub MetroGrid2_CellContentClick(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles MetroGrid2.CellContentClick

    End Sub

    Private Sub MetroTextBox4_Click(sender As System.Object, e As System.EventArgs) Handles MetroTextBox4.Click

    End Sub

    Private Sub MetroTextBox7_Click(sender As System.Object, e As System.EventArgs) Handles MetroTextBox7.Click

    End Sub

    Private Sub MetroTextBox6_Click(sender As System.Object, e As System.EventArgs) Handles MetroTextBox6.Click
