...
#End Region
    ...
    Private Sub Form73_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub MetroButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MetroButton1.Click
        ...
        Try
            ...
            Dim xResumenComprobante As String = ""
            ...
            insSql = "Select * from Cuenta_corriente_proveedores Where (Estado = 0) AND (Cliente=" & xCuenta & ") "
            ...
            If drRecordSet.HasRows = True Then 'Tiene filas
                Do While drRecordSet.Read
                    ...
                    registro("Numero7") = Convert.ToDouble(drRecordSet("Descuentos").ToString)
                    registro("Numero8") = Convert.ToDouble(drRecordSet("Devoluciones").ToString)
                    ...
                    If Val(drRecordSet("DebCre").ToString) = 1 Then
                        registro("Numero2") = 1
                        xSaldo = xSaldo + (Convert.ToDouble(drRecordSet("Importe").ToString) - Convert.ToDouble(drRecordSet("Retenciones").ToString))
                        registro("Numero3") = (Convert.ToDouble(drRecordSet("Importe").ToString) - Convert.ToDouble(drRecordSet("Retenciones").ToString))
                    Else
                        registro("Numero2") = 2
                    ...
                    If Val(drRecordSet("CodigoMovimiento").ToString) = 4 Then
                        xResumenComprobante = resumen_de_pago(xCuenta, drRecordSet("NumeroComprobante").ToString)
                        registro("Literal6") = xResumenComprobante
                    End If
            ...
            Dim mi_CryResumen As New CrResumenProveedores()
    ...
    End Sub
    Private Function resumen_de_pago(ByVal xProveedor As Double, ByVal xRecibo As String) As String
        Dim r As String = ""
        Try
            ...
            Dim InsSql As String = ""
            InsSql = "Select * from Cancelaciones_corriente_Proveedores Where (Cliente = " & xProveedor & ") AND (LetraRecibo='R') AND (NumeroRecibo='" & xRecibo & "') AND (Estado=0) "
            ...
            If drBuscar.HasRows = True Then 'Tiene filas
                Do While drBuscar.Read
                    a = FormatDateTime(drBuscar("Fecha").ToString, DateFormat.ShortDate)
                    If Val(drBuscar("CodigoMovimiento").ToString) = 1 Then
                        b = "FA"
                    End If
                    If Val(drBuscar("CodigoMovimiento").ToString) = 2 Then
                        b = "ND"
                    End If
                    If Val(drBuscar("CodigoMovimiento").ToString) = 3 Then
                        b = "NC"
                    End If
                    c = drBuscar("LetraComprobante").ToString
                    d = drBuscar("NumeroComprobante").ToString
                    e = FormatNumber(drBuscar("Importe").ToString, 2)
                    f = FormatNumber(drBuscar("Cancelado").ToString, 2)

                    g = a & " " & b & " " & c & " " & d & " Imp.: " & e & " Canc. : " & f & " "
    ...
    Private Sub datos_cliente(ByVal xCliente As Double, ByRef xNombreCliente As String, ByRef xComercial As String, _
        ...
        Try
            ...
            Dim InsSql As String = ""
            InsSql = "Select * from Proveedores Where Numero = " & xCliente & "  "
